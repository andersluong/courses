/**
* This solves the minimum cut in a graph problem using the Edmond-Karp algorithm extended with another BFS
* to find the first cut on the graph. Minimum cut is where flow is maximized.
...
void minCut(Graph &graph, Node* const sourceNode, Node* const sinkNode) {
  edmondKarp(graph, sourceNode, sinkNode);

  // find first cut
  std::vector<Node*> nodesInCut;
  ...
  while (!queue.empty()) {
    ...
    queue.pop();
    nodesInCut.push_back(currentNode);
    for (auto edge : currentNode->getEdges()) {
      Node* neighbor = edge->toNode;
      if (!neighbor->isVisited() && edge->capacity > edge->flow) {
        neighbor->setVisited(true);
        queue.push(neighbor);
      }
    }
  }
  std::cout << nodesInCut.size() << "\n";
  for (auto node : nodesInCut) {
...
int main() {
  ...
  minCut(graph, sourceNode, sinkNode);
}
