  ...
  for (int i = 0; i < numberOfTestCases; i++) {
    ...
    for (int i = 0; i < numberOfNodes; i++) {
      for (int j = i + 1; j < numberOfNodes; j++) {
        double weight = std::sqrt((nodes[i].x - nodes[j].x) * (nodes[i].x - nodes[j].x) + (nodes[i].y - nodes[j].y) * (nodes[i].y - nodes[j].y));
        graph.push_back(Edge{weight, i, j});
      }
    }

    double totalWeight = minSpanTree(graph, numberOfNodes);
    ...
    // print the result
    std::cout << std::fixed << std::setprecision(2) << totalWeight << '\n';
  }
}
